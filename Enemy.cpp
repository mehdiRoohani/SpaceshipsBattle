...
Enemy::Enemy(Texture* image, Point pos, float ang,int id):Unit(image,pos,id){
...
void Enemy::loadTurret(int index,int type){
...
void Enemy::Rotate(int direction){
...
void Enemy::updateHealth(float healthValue){
    health=healthValue;
    hBar->updateHealth(health);
}

bool Enemy::selectTarget(Unit* enemy){
   for(int i=0; i<8; i++){
        if(turretPosition[i])
        {
...
Unit** Enemy::fire(long int& frame,int id){
    for(int i=0; i<8; i++){
...
void Enemy::Render(long int& frame, SDL_Renderer* gRenderer){
    image->Render(position.x-(spriteClips[0].w)/2, position.y-(spriteClips[0].h)/2, &spriteClips[frame%2], angle, NULL, SDL_FLIP_NONE, gRenderer );
